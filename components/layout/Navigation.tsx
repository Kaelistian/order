    ...
    <header className="fixed left-0 right-0 top-0 z-50 px-4 pt-4 sm:px-6">
      <motion.nav
        ...
        className={`mx-auto flex max-w-6xl items-center justify-between rounded-full border px-4 py-3 transition duration-300 sm:px-5 ${
          hasScrolled
            ? "border-white/10 bg-black/56 shadow-2xl shadow-white/10 backdrop-blur-2xl"
      ...
      >
        <a
          href="#home"
            className="group inline-flex items-center gap-3 rounded-full outline-none focus-visible:ring-2 focus-visible:ring-white/70 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
        ...
        <div className="hidden items-center gap-1 md:flex">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="rounded-full px-4 py-2 text-sm text-white/58 outline-none transition hover:bg-white/[0.055] hover:text-white focus-visible:ring-2 focus-visible:ring-white/70"
            >
              {item.label}
            </a>
          ))}
        ...
        <button
          type="button"
          className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-white outline-none transition hover:bg-white/[0.08] focus-visible:ring-2 focus-visible:ring-white/70 md:hidden"
        ...
        >
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="rounded-[18px] px-4 py-3 text-sm text-white/70 outline-none transition hover:bg-white/[0.06] hover:text-white focus-visible:ring-2 focus-visible:ring-white/70"
              onClick={() => setIsOpen(false)}
            >
              {item.label}
            </a>
